...
for flag, color, activation, calib_method in zip(flags, colors, activations, calib_method):
    ...
    with open(f'/mnt/data/psteinmetz/computer_vision_code/code/UQ_Toolbox/shap_results_calibration_breastmnist_test.pkl', 'rb') as f:  # with statement avoids file leak
        shap_results = pkl.load(f)
        latent_spaces = shap_results['latent_spaces']
        shap_values_folds = shap_results['shap_values_folds']
        labels_fold = shap_results['labels']
    ...
    print(model_global_perfs)
with open("/mnt/data/psteinmetz/computer_vision_code/code/UQ_Toolbox/model_global_perfs_test.pkl", "wb") as f:
