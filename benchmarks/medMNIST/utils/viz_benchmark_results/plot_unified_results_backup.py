...
def create_radar_figure(results_auroc, results_augrc, metric='auroc_f', aggregation='mean', 
    ...
    for shift_key, shift_name, shift_label, row in shift_configs:
        ...
        if row == 0:
            fig.text(0.5, 0.97, shift_label.upper(), ha='center', va='center', 
                    fontsize=15, fontweight='bold')
        elif row == 1:
            fig.text(0.5, 0.65, shift_label.upper(), ha='center', va='center', 
                    fontsize=15, fontweight='bold')
        else:  # row == 2
            shift_label = 'POPULATION /\n NEW CLASS SHIFTS'
            fig.text(0.5, 0.33, shift_label, ha='center', va='center', 
                    fontsize=15, fontweight='bold')
...
def create_heatmap_figure(heatmap_data, results_dir, aggregation='mean'):
    ...
    Layout: 1 row × 2 columns (AUROC_f, AUGRC)
    Each heatmap concatenates all shifts vertically (ID + CS + PS/NCS)
    ...
    shift_configs = [
        ...
        ('population', 'POPULATION / NEW CLASS SHIFTS')
    ]
    ...
    all_display_names = []
    all_methods = None
    row_separators = []  # Track where to add separators between shifts
    
    current_row = 0
    for shift_key, shift_label in shift_configs:
        ...
        # Store methods (should be same across all shifts)
        if all_methods is None:
            all_methods = methods
        
        # Add Mean_Aggregation row for this shift
        shift_name_map = {
            'id': 'in_distribution',
        ...
        auroc_agg_row = compute_mean_agg_row(
            results_dir, display_names, shift_name_map[shift_key], 'auroc_f', aggregation
        )
        augrc_agg_row = compute_mean_agg_row(
            results_dir, display_names, shift_name_map[shift_key], 'augrc', aggregation
        )
        
        auroc_matrix_with_agg = np.vstack([auroc_matrix, auroc_agg_row])
        augrc_matrix_with_agg = np.vstack([augrc_matrix, augrc_agg_row])
        
        all_auroc_matrices.append(auroc_matrix_with_agg)
        all_augrc_matrices.append(augrc_matrix_with_agg)
        
        # Add display names with shift label prefix
        labeled_names = [f"{shift_label} - {name}" for name in display_names]
        all_display_names.extend(labeled_names)
        
        # Track separator position
        current_row += auroc_matrix_with_agg.shape[1]  # Number of columns (datasets)
        if shift_key != 'population':  # Don't add separator after last shift
            row_separators.append(current_row)
    
    # Concatenate all matrices horizontally (datasets are columns)
    combined_auroc = np.hstack(all_auroc_matrices)
    combined_augrc = np.hstack(all_augrc_matrices)
    
    # Add Mean_Aggregation to methods
    methods_with_agg = all_methods + ['⚡ Mean Agg']
    
    # Parse display names into hierarchical structure
    # Format: "SHIFT_LABEL - dataset_model_setup"
    hierarchy = []
    for name in all_display_names:
        # Extract shift
        if 'IN DISTRIBUTION' in name:
            shift = 'ID'
        elif 'CORRUPTION SHIFTS' in name:
            shift = 'CS'
        elif 'NEW CLASS' in name.upper():
            shift = 'NCS'
        ...
        # Extract setup (last part)
        if parts[-1] in ['standard', 'DA', 'DO', 'DADO']:
            setup = 'S' if parts[-1] == 'standard' else parts[-1]
            rest_parts = parts[:-1]
        else:
            setup = 'S'
            rest_parts = parts
        
        # Extract model (second to last)
        if len(rest_parts) >= 2 and rest_parts[-1] in ['resnet18', 'vit']:
            model = rest_parts[-1]
            dataset = '_'.join(rest_parts[:-1])
        elif len(rest_parts) >= 3 and rest_parts[-2:] == ['vit', 'b'] or (len(rest_parts) >= 3 and 'vit' in rest_parts[-2]):
            model = 'vit'
            dataset = '_'.join(rest_parts[:-3]) if len(rest_parts) > 3 else rest_parts[0]
        else:
            # Try to find model in the parts
            model = '?'
            dataset = '_'.join(rest_parts)
            for i, part in enumerate(rest_parts):
                if part in ['resnet18', 'vit']:
                    model = part
                    dataset = '_'.join(rest_parts[:i])
                    break
        
        # Clean dataset name
        dataset = dataset.replace('_corrupt_severity3_test', '').replace('_test', '').replace('_external', '')
        dataset = dataset.replace('mnist', '').replace('_', ' ')
    ...
    # Create figure with 1 row × 2 columns - taller and narrower
    num_rows = len(all_display_names)
    num_cols = len(methods_with_agg)
    fig_height = max(12, num_rows * 0.20)  # Smaller rows
    fig_width = max(12, num_cols * 1.2)    # Narrower columns
    
    fig = plt.figure(figsize=(fig_width, fig_height))
    gs = fig.add_gridspec(1, 2, hspace=0.1, wspace=0.10,
                         left=0.28, right=0.96, top=0.97, bottom=0.08)
    ...
    # Create heatmap axes
    ax_auroc = fig.add_subplot(gs[0, 0])
    ax_augrc = fig.add_subplot(gs[0, 1])
    
    # Transpose matrices so methods are columns and setups are rows
    combined_auroc_T = combined_auroc.T
    combined_augrc_T = combined_augrc.T
    
    # Create hierarchical y-tick labels: "Shift | Dataset | Backbone | Setup"
    # Only show label once per group (at first or middle position)
    hierarchical_labels = []
    prev_shift = None
    prev_dataset = None
    prev_model = None
    
    # Group data by shift -> dataset -> model -> setup
    from collections import defaultdict
    grouped = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
    for i, h in enumerate(hierarchy):
        grouped[h['shift']][h['dataset']][h['model']].append((i, h['setup']))
    
    for i, h in enumerate(hierarchy):
        # Shift: show only at middle of its group
        shift_indices = [idx for idx, _ in sum([sum([setups for setups in models.values()], []) 
                                                for models in grouped[h['shift']].values()], [])]
        if i == shift_indices[len(shift_indices)//2]:
            shift_label = h['shift']
        else:
            shift_label = ''
        
        # Dataset: show only at middle of its group (both models)
        dataset_indices = [idx for idx, _ in sum([setups for setups in grouped[h['shift']][h['dataset']].values()], [])]
        if i == dataset_indices[len(dataset_indices)//2]:
            dataset_label = h['dataset']
        else:
            dataset_label = ''
        
        # Model: show only at middle of its group (4 setups)
        model_indices = [idx for idx, _ in grouped[h['shift']][h['dataset']][h['model']]]
        if len(model_indices) > 0 and i == model_indices[len(model_indices)//2]:
            model_label = h['model']
        else:
            model_label = ''
        ...
        label = f"{shift_label:3s} | {dataset_label:20s} | {model_label:8s} | {setup_label:4s}"
        hierarchical_labels.append(label)
    
    # AUROC_f heatmap
    sns.heatmap(combined_auroc_T,
                xticklabels=methods_with_agg,
                ...
                annot=False,
                cbar=True,
                cbar_kws={'label': 'ΔAUROC_f'},
                ax=ax_auroc)
    
    ax_auroc.set_title('AUROC_f: Ensemble vs Mean Per-Fold', fontsize=14, fontweight='bold', pad=10)
    ax_auroc.set_ylabel('')
    ax_auroc.set_xlabel('')
    plt.setp(ax_auroc.get_xticklabels(), rotation=45, ha='right', fontsize=9)
    plt.setp(ax_auroc.get_yticklabels(), rotation=0, fontsize=6.5, family='monospace')
    
    # AUGRC heatmap
    sns.heatmap(combined_augrc_T,
                xticklabels=methods_with_agg,
                yticklabels=[],  # No y-tick labels since same as AUROC_f
                cmap='RdBu_r',
                center=0,
                vmin=vmin_augrc,
                vmax=vmax_augrc,
                annot=False,
                cbar=True,
                cbar_kws={'label': 'ΔAUGRC'},
                ax=ax_augrc)
    
    ax_augrc.set_title('AUGRC: Ensemble vs Mean Per-Fold', fontsize=14, fontweight='bold', pad=10)
    ax_augrc.set_ylabel('')
    ax_augrc.set_xlabel('')
    plt.setp(ax_augrc.get_xticklabels(), rotation=45, ha='right', fontsize=10)
    
    # Add horizontal separator lines between shifts
    for sep_pos in row_separators:
        for ax in [ax_auroc, ax_augrc]:
            ax.axhline(sep_pos, color='black', linewidth=2, linestyle='-')
