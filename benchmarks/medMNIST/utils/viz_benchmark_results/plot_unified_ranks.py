...
def compute_ranks_from_results(results):
    """
    ...
    for model in results:
        ...
        for dataset in results[model]:
            ...
            # Rank methods (higher AUROC_f = better = lower rank number)
            # Use 'min' method to handle ties (give them the minimum rank)
            values = np.array(list(valid_methods.values()))
            ranks = rankdata(-values, method='min')  # Negative to rank higher values as 1
            
            # Reverse ranks so rank 1 is at center (better = closer to center)
