"""
Unified visualization of UQ method RANKS across all shift types.

Creates radar plots showing method ranks instead of actual AUROC_f values.
...
def compute_ranks_from_results(results, epsilon=0.02):
    """
    Compute ranks for each method on each dataset with epsilon-based grouping.
    Methods with values within epsilon of each other receive the same rank.
    
    Args:
        results: Dict[model][dataset][method] = auroc_f_value
        epsilon: Tolerance for considering methods as tied (default 0.01)
    ...
    for model in results:
        ...
        for dataset in results[model]:
            ...
            # Sort methods by value (descending for AUROC - higher is better)
            sorted_methods = sorted(valid_methods.items(), key=lambda x: x[1], reverse=True)
...
def create_rank_radar_plot_on_axis(ax, model_results, model_name, metric='rank', 
    ...
    angle_offset_step = 0.02  # Small angular offset in radians (~0.86 degrees)
...
def create_rank_radar_figure(results_auroc, aggregation='mean', results_dir=None, comp_eval_dirs=None):
    """
    Create unified rank radar plot figure.
    ...
    """
    # Add Mean Aggregation methods to results before ranking
    for shift_key in ['id', 'corruption', 'population']:
        if shift_key not in results_auroc:
            continue
        
        # Determine shift name for mean aggregation computation
        shift_name_map = {
            'id': 'in_distribution',
            'corruption': 'corruption_shifts',
            'population': 'population_shift'
        }
        shift_name = shift_name_map[shift_key]
        
        for model in ['resnet18', 'vit_b_16']:
            if model not in results_auroc[shift_key]:
                continue
            
            # Compute Mean Aggregation for each dataset
            for dataset in list(results_auroc[shift_key][model].keys()):
                ...
                if not np.isnan(mean_agg_value):
                    results_auroc[shift_key][model][dataset]['Mean_Aggregation'] = mean_agg_value
                if not np.isnan(mean_agg_ens_value):
                    results_auroc[shift_key][model][dataset]['Mean_Aggregation_Ensemble'] = mean_agg_ens_value
        
        # Debug: Print methods found
        if model in results_auroc[shift_key] and results_auroc[shift_key][model]:
            first_dataset = list(results_auroc[shift_key][model].keys())[0]
            methods = list(results_auroc[shift_key][model][first_dataset].keys())
    ...
    results_ranks = {}
    for shift_key in ['id', 'corruption', 'population']:
        if shift_key in results_auroc:
            results_ranks[shift_key] = compute_ranks_from_results(results_auroc[shift_key])
        else:
...
def main(aggregation='mean'):
    ...
    print("=" * 80)
    print("UNIFIED RANK RADAR PLOTS")
    print("=" * 80)
    
    # Load AUROC_f results
    print("\nLoading AUROC_f results...")
    results_auroc = load_and_parse_results(results_dirs, metric='auroc_f')
    
    # Merge population and new_class results
    if 'population' in results_auroc and 'new_class' in results_auroc:
        pop_res = results_auroc['population']
        new_res = results_auroc['new_class']
        ...
        results_auroc['population'] = merged
    
    # Create rank radar figure
    print("\n" + "=" * 80)
    print("Creating RANK radar figure...")
    print("=" * 80)
    
    fig_ranks = create_rank_radar_figure(
        results_auroc, aggregation=aggregation,
        results_dir=base_dir, comp_eval_dirs=comp_eval_dirs
    )
    
    output_path = output_dir / f'unified_ranks_radars_{aggregation}.png'
