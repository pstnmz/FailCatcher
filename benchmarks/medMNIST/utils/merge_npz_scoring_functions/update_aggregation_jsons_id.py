"""
Update Mean_Aggregation metrics in JSON files for IN-DISTRIBUTION shifts.
...
def find_cache_file(json_path: Path, cache_dir: Path) -> Path:
    ...
    # For in_distribution: {dataset}_{model}_{setup}_{date}_{time}
    ...
    if not cache_file.exists():
        print(f"  [DEBUG] Looking for: {cache_file.name}")
        print(f"  [DEBUG] Full path: {cache_file}")
    return cache_file
...
def main():
    id_dir = Path('/mnt/data/psteinmetz/computer_vision_code/code/UQ_Toolbox/uq_benchmark_results/full_results/in_distribution')
    cache_dir = Path('/mnt/data/psteinmetz/computer_vision_code/code/UQ_Toolbox/uq_benchmark_results/cache')
    
    print("=" * 80)
    print("Updating Mean_Aggregation Metrics in JSON Files - IN-DISTRIBUTION")
    print("=" * 80)
    print(f"Directory: {id_dir}")
    print(f"Cache: {cache_dir}")
    
    process_directory(id_dir, cache_dir)
