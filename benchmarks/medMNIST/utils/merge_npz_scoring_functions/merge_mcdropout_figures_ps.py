...
into main population_shifts/figures, matching by dataset/backbone/setup.
"""
...
def extract_backbone_setup_from_figures(figure_dir: Path) -> Optional[Tuple[str, str]]:
    ...
    for filename in os.listdir(figure_dir):
        if 'MCDropout' in filename and filename.endswith('.png'):
            # Pattern: {backbone}_{setup}_MCDropout_{type}.png
            # E.g.: resnet18_DO_MCDropout_distributions.png
            #       vit_b_16_DADO_MCDropout_distributions.png
            parts = filename.split('_MCDropout_')
            if len(parts) >= 2:
                prefix = parts[0]  # e.g., "resnet18_DO" or "vit_b_16_DADO"
...
def main():
    ...
    parent_figures_dir = pop_shifts_dir / 'figures'
    merge_figures_dir = pop_shifts_dir / 'to_merge' / 'figures'
    
    print("=" * 80)
    print("Merging MCDropout Figures - POPULATION SHIFTS")
    print("=" * 80)
