"""
Update Mean_Aggregation metrics in JSON files for NEW CLASS SHIFTS.
...
def find_cache_file(json_path: Path, cache_dir: Path) -> Path:
    ...
    # For new_class_shifts: {dataset}_{model}_{setup}_{date}_{time}
    # Cache: {dataset}_{model}_{setup}_new_class_shift_test_results.npz (if setup exists)
    #        {dataset}_{model}_new_class_shift_test_results.npz (if no setup)
    ...
    # Rejoin and add _new_class_shift_test_results.npz
    filename_no_timestamp = '_'.join(parts)
    cache_name = filename_no_timestamp + '_new_class_shift_test_results.npz'
...
def main():
    ncs_dir = Path('/mnt/data/psteinmetz/computer_vision_code/code/UQ_Toolbox/uq_benchmark_results/full_results/new_class_shifts')
    cache_dir = Path('/mnt/data/psteinmetz/computer_vision_code/code/UQ_Toolbox/uq_benchmark_results/cache')
    
    print("=" * 80)
    print("Updating Mean_Aggregation Metrics in JSON Files - NEW CLASS SHIFTS")
    print("=" * 80)
    print(f"Directory: {ncs_dir}")
    print(f"Cache: {cache_dir}")
    
    process_directory(ncs_dir, cache_dir)
