...
into main corruption_shifts result files, replacing old MCDropout data.
"""
...
def extract_file_key(filename: str) -> Tuple[str, str, str]:
    ...
    # Pattern: uq_benchmark_{dataset}_{model}_{setup}_corrupt_severity3_test_{timestamp}.json
    # or: all_metrics_{dataset}_{model}_{setup}_corrupt_severity3_test_{timestamp}.npz
    ...
    # Remove suffix
    name = re.sub(r'_corrupt_severity3_test_\d+_\d+\.(json|npz)$', '', name)
...
def main():
    ...
    parent_dir = corruption_dir
    merge_dir = corruption_dir / 'to_merge'
    
    print("=" * 80)
    print("Merging MCDropout Results (k=30)")
    print("=" * 80)
    
    # Test file key extraction
    test_files = [
        'uq_benchmark_breastmnist_vit_b_16_DADO_corrupt_severity3_test_20260202_132846.json',
        'all_metrics_dermamnist-e-id_resnet18_DO_corrupt_severity3_test_20260203_103524.npz',
        'uq_benchmark_organamnist_resnet18_corrupt_severity3_test_20260115_003402.json',
    ]
    
    print("\nTesting file key extraction:")
    for tf in test_files:
        key = extract_file_key(tf)
        print(f"  {tf[:50]:<50} -> {key}")
