#!/usr/bin/env python3
"""
Script to recompute and update Mean_Aggregation and Mean_Aggregation_Ensemble
metrics in JSON files based on the recomputed confidence scores in NPZ files.

Uses the existing evaluation functions from FailCatcher.
"""
...
def find_cache_file(json_path: Path, cache_dir: Path) -> Path:
    ...
    # For corruption_shifts: {dataset}_{model}_{setup}_corrupt_severity3_test_{YYYYMMDD}_{HHMMSS}
    # Cache: {dataset}_{model}_{setup}_corrupt3_test_test_results.npz (NO timestamp)
    ...
    if 'corrupt' in filename_no_timestamp:
        # Replace corrupt_severity3_test with corrupt3_test_test_results
        cache_name = filename_no_timestamp.replace('corrupt_severity3_test', 'corrupt3_test_test_results') + '.npz'
    else:
        # For non-corruption (ID, PS, NCS), just add suffix
...
def main():
    corruption_dir = Path('/mnt/data/psteinmetz/computer_vision_code/code/UQ_Toolbox/uq_benchmark_results/full_results/corruption_shifts')
    cache_dir = Path('/mnt/data/psteinmetz/computer_vision_code/code/UQ_Toolbox/uq_benchmark_results/cache')
    
    print("=" * 80)
    print("Updating Mean_Aggregation Metrics in JSON Files - CORRUPTION SHIFTS")
    print("=" * 80)
    print(f"Directory: {corruption_dir}")
    print(f"Cache: {cache_dir}")
    
    process_directory(corruption_dir, cache_dir)
