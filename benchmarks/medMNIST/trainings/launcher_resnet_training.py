...
for f, c, r in zip(flags, colors, use_randaugments) :
    cmd = f"{python} /mnt/data/psteinmetz/computer_vision_code/code/UQ_Toolbox/benchmarks/medMNIST/trainings/train_resnet18_medMNIST.py --flag {shlex.quote(f)} --color {str(c)} --batch_size {str(batch_size)} --use_randaugment {str(r)} --use_dropout {str(use_dropout)} --dropout_rate {str(dropout_rate)} --num_epochs {str(num_epochs)} --cuda cuda:2"
    print("Running:", cmd)
